...
uniform float EnvScale : register(c20);
...
vpconn main(appdata IN)
{
	...
	OUT.WorldTangent.xyz = mul((float3x3)modelToWorld, IN.Tangent);
	OUT.WorldTangent.w = IN.Tangent.w;
	OUT.WorldNormal = mul((float3x3)modelToWorld, IN.Normal);
