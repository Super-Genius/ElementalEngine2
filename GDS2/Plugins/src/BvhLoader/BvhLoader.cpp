...
AFX_EXTENSION_MODULE BvhLoaderDLL = { NULL, NULL };
CBvhLoader *gBvhLoader;
...
extern "C" int APIENTRY DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("BvhLoader.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(BvhLoaderDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(BvhLoaderDLL);
		// this has to be done after module is intialized.
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("BvhLoader.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(BvhLoaderDLL);
		delete gBvhLoader;
	}
	return 1;   // ok
}
...
{
	gBvhLoader = new CBvhLoader(pi);
	return gBvhLoader;
}

static LPTSTR OpenFilter = {
	"Motion Capture BVH (*.bvh)\0" "*.bvh\0"
	"\0\0"
};
...
// Returns:		.
//---------------------------------------------------------------------
CBvhLoader::CBvhLoader(CProgramInfo *ProgramInfo)
{
...
//---------------------------------------------------------------------
CString *CBvhLoader::GetName()
{
    static CString name;
    LPTSTR tBuff;

    tBuff = name.GetBuffer(_MAX_PATH+1);
	GetModuleFileName(BvhLoaderDLL.hModule, tBuff, _MAX_PATH);
    name.ReleaseBuffer();
...
BOOL CBvhLoader::Initialize()
{
	...
	AfxSetResourceHandle(BvhLoaderDLL.hModule);
	...
	m_DocTemplate = new CMultiDocTemplate(IDR_TEXTURETYPE,
			RUNTIME_CLASS(CBvhDoc),
			RUNTIME_CLASS(CMDIChildWnd),
			RUNTIME_CLASS(CBvhView));
	...
	m_ObjInfo.icon = (HICON)LoadImage(BvhLoaderDLL.hModule, MAKEINTRESOURCE(IDR_TEXTURETYPE), 
...
CBvhLoader::~CBvhLoader()
{
	...
	for (toIT = m_BvhObjects.begin(); toIT != m_BvhObjects.end(); toIT++)
	{
		delete *toIT;
	}
...
//---------------------------------------------------------------------
DWORD CBvhLoader::Save(LPTSTR filename)
{
	return NO_ERROR;
}
...
// Returns:		DWORD = possible error or 0 for no error
//---------------------------------------------------------------------
DWORD CBvhLoader::Load(LPTSTR filename)
{
