...
{
public:
	virtual BOOL BroadcastMessage(UINT nID, void* pExtra) = 0;
	virtual PFNCLASSOVERRIDE GetOverrideClass(LPCTSTR className) = 0;
	virtual void SetOverrideClass(OVERRIDECLASS *overrideInfo) = 0;
};
...
public:
	...
	// Pure virtual base pointer passed to the dll
	// interface so it can call back to application
	CCallbackInterface *cbi;
};
...
extern PFNCLASSOVERRIDE FindClassOverride(LPCTSTR className);
