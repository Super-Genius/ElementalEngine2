///============================================================================
/// \file		AppertureGenRenderObject.h
/// \brief		Header file for an Apperture Generator Render Object
/// \date		09-11-2007
...
#ifndef	APPERTUREGENRENDEROBJECT_H
#define	APPERTUREGENRENDEROBJECT_H

#include "CObjectTemplate.hpp"
#include "CRenderObject.h"

struct GENERATEAPPERTURETEXTUREPARAMS
{
	IBaseTextureObject *pSourceTexture;
	IHashString *pDestTextureName;
	int iDestTextureSize;
	float fOffset;
	float fFlatten;
};

class CAppertureGenRenderObject : public CRenderObject<>
{
protected:
	/// \brief Default Constructor
	/// \param parentName = name of parent in hierarchy
	/// \param name = IHashString * name of this object
	CAppertureGenRenderObject(IHashString *parentName, IHashString *name, bool bAddToHier);

public:
	/// \briefDefault Destructor
	virtual ~CAppertureGenRenderObject();
	...
	// Messages
	DWORD OnGenerateAppertureTexture( DWORD size, void * params );

private:
	void CreateTextures( int iRenderTargetSize );
	void DestroyTextures();
	void CreateTextureGenerationGeometry();
	void DestroyTextureGenerationGeometry();
	void GenerateAppertureTexture( IHashString *pDestTextureName );
	...
	IEffect *m_pEffect;
	IBaseTextureObject *m_pSourceTexture;
	IBaseTextureObject *m_pZeroTexture;
	ITextureObject *m_pAccumRenderTargets[2];
	...
	int m_iAccumRenderTarget;
	int m_iAccumPass;
	int m_iRenderTargetSize;
	float m_fOffset;
	float m_fFlatten;
};

#endif //APPERTUREGENRENDEROBJECT_H
