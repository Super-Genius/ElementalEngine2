// PSDialog.cpp : implementation file
//
...
BEGIN_MESSAGE_MAP(CPSDialog, CDialog)
	ON_BN_CLICKED(IDC_DIRECTORY_SEL, &CPSDialog::OnBnClickedDirectorySel)
	ON_BN_CLICKED(IDC_BUTTON_LISTBOX_ADD, &CPSDialog::OnBnClickedButtonListboxAdd)
	ON_BN_CLICKED(IDC_BUTTON_LISTBOX_REMOVE, &CPSDialog::OnBnClickedButtonListboxRemove)
	ON_BN_CLICKED(IDC_BUTTON_STARTUP_SCRIPT_SELECTOR, &CPSDialog::OnBnClickedButtonStartupScriptSelector)
END_MESSAGE_MAP()

// CPSDialog dialog

IMPLEMENT_DYNAMIC(CPSDialog, CDialog)

CPSDialog::CPSDialog(CWnd* pParent /*=NULL*/)
	: CDialog(CPSDialog::IDD, pParent)
{
...
void CPSDialog::DoDataExchange(CDataExchange* pDX)
{
...
// CPSDialog message handlers

void CPSDialog::OnBnClickedDirectorySel()
{
	TCHAR szPathBuffer[MAX_PATH];
	CProjectSettings *projSettings = SINGLETONINSTANCE(CProjectSettings);

	BROWSEINFO bi = { 0 };
	bi.lpszTitle = _T("Pick A Project Directory");

	_tcsncpy(szPathBuffer, projSettings->GetDirectory(), MAX_PATH);
...
void CPSDialog::OnOK()
{
	...
	//TODO:Validating other values on the form
	...
	// we have just created a new project settings the project
	// file name needs to be reset
	if ( m_bAllowNew )
	{
		m_projectSettings->setSaveFileName( "" );
	}

	//MOVED TO CProjectSettins.cpp
	//// if the project directory is changed, we need to update it in GDS
	//if ( bDirHasChanged ) 
	//{
	//	// TODO: PROJECT DIRECTORY UPDATE NEEDED
	//	//theApp.SetResourcePath( newDirectory );
	//}

	CDialog::OnOK(); // This will close the dialog and DoModal will return.
}

bool CPSDialog::regExpValidate(CString p_validateThis, CString p_pattern, CString p_errorString)
{
...
void CPSDialog::OnBnClickedButtonListboxAdd()
{
...
void CPSDialog::OnBnClickedButtonListboxRemove()
{
...
void CPSDialog::OnBnClickedButtonStartupScriptSelector()
{
	// get the project directory string
	CString projectDirString;	
	m_ProjectDirEdit.GetWindowText( projectDirString );
...
void CPSDialog::EnableInputs ( bool bEnable ) 
{
