...
IMPLEMENT_DYNAMIC(COptionTreeDock, CGuiControlBar)
COptionTreeDock::COptionTreeDock(StdString formatFile, StdString formatDir)
{	
...
COptionTreeDock::~COptionTreeDock()
{
...
BEGIN_MESSAGE_MAP(COptionTreeDock, CGuiControlBar)
	ON_WM_CREATE()
	ON_NOTIFY(OT_NOTIFY_ITEMCHANGED, IDC_GUIEDITOR_OPTIONTREE, OnUpdateProperties)
END_MESSAGE_MAP()

void COptionTreeDock::UpdateOptionTree(IHashString *name, IHashString *compType)
{
...
void COptionTreeDock::UpdateOptionTree(vector<CHashString>names, vector<CHashString>types)
{
...
void COptionTreeDock::ClearOptionTree(void)
{
	m_OptionTreeView->CollapseAll();
}

int COptionTreeDock::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	StdString basePath;
	m_ToolBox->GetDirectories(&basePath, NULL);

	basePath += m_szFormatDir;
	...
	m_OptionTreeView = new COptionTreeWrapper(dwStyle, rcClient, this, dwOptions, IDC_GUIEDITOR_OPTIONTREE, basePath.c_str());

	m_OptionTreeView->Init(m_szFormatFile);

	return 0;
}

afx_msg void COptionTreeDock::OnUpdateProperties(NMHDR* pNotifyStruct, LRESULT* plResult)
{
