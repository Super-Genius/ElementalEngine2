///==========================================================================
/// \file	CBoundaryPhysicsObject.cpp
...
// ------------- Register Object ---------------- //
REGISTER_COMPONENT(CBoundaryPhysicsObject);

CBoundaryPhysicsObject::CBoundaryPhysicsObject( IHashString *parentName, IHashString *name, bool addToHier ) : 
...
CBoundaryPhysicsObject::~CBoundaryPhysicsObject()
{
	DeInit();
...
IComponent* CBoundaryPhysicsObject::Create( int nArgs, va_list argptr )
{
	IHashString *name;
	...
	try
	{
		if( nArgs == 2 )
		{
			self = new CBoundaryPhysicsObject( parentName, name, true );
		}
		else if( nArgs == 3 )
		{
			bAddToHier = va_arg( argptr, bool );
			self = new CBoundaryPhysicsObject( parentName, name, bAddToHier );
		}
		else
		{
	...
	{
		EngineGetToolBox()->Log( LOGERROR, _T("CBoundaryPhysicsObject: Failed to create CBoundaryPhysicsObject!\n") );
		return NULL;
	}
...
void CBoundaryPhysicsObject::Init()
{
...
bool CBoundaryPhysicsObject::Update()
{
	// boundarys are always static
...
void CBoundaryPhysicsObject::DeInit()
{
...
void CBoundaryPhysicsObject::Serialize( IArchive &ar )
{
	if( ar.IsReading() )
	{
...
IHashString* CBoundaryPhysicsObject::GetComponentType()
{
	static CHashString cpo( _T("CBoundaryPhysicsObject") );
	return &cpo;
}

bool CBoundaryPhysicsObject::IsKindOf(IHashString *compType)
{
...
void CBoundaryPhysicsObject::SetTransform()
{
	// Can't set the transform on static meshes
}

void CBoundaryPhysicsObject::BuildBoundaryActor()
{
	// Make sure that we have a valid mesh
	if( m_Verts == NULL || m_Faces == NULL || m_NumVerts == 0 || m_NumFaces == 0 )
	{
		EngineGetToolBox()->Log( LOGERROR, _T("Invalid Mesh data to build CBoundaryPhysicsObject!\n") );
	}
	...
	if( status == false )
	{
		EngineGetToolBox()->Log( LOGERROR, _T("Failed to cook mesh data in CBoundaryPhysicsObject::BuildboundaryActor()!\n") );
		NxCloseCooking();
		return;
	}
	NxCloseCooking();
...
void CBoundaryPhysicsObject::PopulateVertexData()
{
	CREATEARCHIVE ca;
	...
	if( retVal == MSG_HANDLED && gv.archive != NULL )
	{
		...
		{
			m_ToolBox->Log( LOGERROR, _T( "CBoundaryPhysicsObject sent unhandled message 'GetVertices'\n") );
		}
	...
	{
		m_ToolBox->Log( LOGERROR, _T( "CBoundaryPhysicsObject could not create archive for in PopulateVertexData()\n") );
	}
}