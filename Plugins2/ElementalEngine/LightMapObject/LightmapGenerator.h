...
public:
    DWORD OnAddMesh( DWORD size, void * params )
	{
		...
		//TODO FIX
		MeshParameterization * mesh =  msg->mesh;
...
private:
	...
	bool ComputeIrradianceAtPoint( Vec3 &tcoord,  Vec3 &normal, float &maxLightIntensity, floatColor &outDirect,
	...
	vector< MeshParameterization * > m_MeshObjects;
	vector< ITextureObject * > m_TextureObjects;
	vector< ILightObject * > m_Lights;
	KDTree * m_KDTree;
	Vec3 * m_KDBuffer;//until kdtree is more optimal
	PhotonValue * m_KDValue;
	int m_KDCounter;
