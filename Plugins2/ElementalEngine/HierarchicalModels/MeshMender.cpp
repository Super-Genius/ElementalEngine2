...
{
	static bool operator<(const Vec3& first, const Vec3& second)
	{
...
{
public:
	virtual Smoothing CanSmooth(MeshMender::Triangle* t1, MeshMender::Triangle* t2, const float& minCreaseAngle)=0;
};
...
{
public:
    virtual Smoothing CanSmooth(MeshMender::Triangle* t1, MeshMender::Triangle* t2, const float& minCreaseAngle)
	{
        ...
        const float angle = tmp1.Dot(tmp2);
...
/*       
if ( fabsf( angle ) >= minCreaseAngle )
        {
            return Smooth_Invert;
        }*/
...
public:
	virtual Smoothing CanSmooth(MeshMender::Triangle* t1, MeshMender::Triangle* t2, const float& minCreaseAngle)
	{
		...
		//for checking the angle, we want these to be normalized,
		//they may not be for whatever reason
		Vec3 tmp1 = t1->tangent;
		Vec3 tmp2 = t2->tangent;
		tmp1.Normalize();
		tmp2.Normalize();
...
public:
	virtual Smoothing CanSmooth(MeshMender::Triangle* t1, MeshMender::Triangle* t2, const float& minCreaseAngle)
	{
		...
		//for checking the angle, we want these to be normalized,
		//they may not be for whatever reason
		Vec3 tmp1 = t1->binormal;
		Vec3 tmp2 = t2->binormal;
		tmp1.Normalize();
		tmp2.Normalize();
								...
								Vec3 workingPosition)
{
		...
		for( size_t i = 0; i < possibleNeighbors.size(); ++i )
		{
		...
		for( size_t i = 0; i < neighborGroups.size(); ++i )
		{
		...
		for( size_t i = 0; i < neighborGroups.size(); ++i )
		{
			...
			for( size_t t = 0; t < curGroup.size(); ++t ) //for each tri
			{
                ...
                const Triangle& tri = m_Triangles[ tID ];

                const size_t vert_size = theVerts.size();

                assert( tri.myID < m_Triangles.size() );

                assert( tri.handled );
                assert( tri.indices[ 0 ] < vert_size );
                assert( tri.indices[ 1 ] < vert_size );
                assert( tri.indices[ 2 ] < vert_size );


				for(size_t indx = 0; indx < 3 ; ++indx)//for each vert in that tri
				{
                    const unsigned int this_index = tri.indices[ indx ];
    
                    assert( this_index < vert_size );

					//if it is at the positions in question
					if( theVerts[ this_index ].pos  == workingPosition )
					{
                        assert( i < groupNormalVectors.size() );

						//see if another group is already using this vert
						if(otherGroupsIndices.find( this_index ) != otherGroupsIndices.end() )
						{
							//then we need to make a new vertex
							Vertex ov;
							ov = theVerts[ this_index ];
							ov.normal = groupNormalVectors[i];
							size_t oldIndex = this_index;
						...
						else
						{
							...
							theVerts[ this_index ].normal = groupNormalVectors[i];
						}

						//store that we have used this index, so other groups can check
						thisGroupIndices.insert( this_index);
					}
				}
								...
								Vec3 workingPosition)
{
		...
		for( size_t i=0; i<neighborGroups.size(); ++i)
		{
			...
			for(unsigned int t = 0; t < neighborGroups[i].size(); ++t)//for each triangle in the group, 
			{
                ...
                if ( flipGroups[ i ][ t ] == Smooth_Invert )
                {
								...
								const float& minCreaseAngle)
{
	...
	{
        assert( neighbor1->group < neighborGroups.size() );

        Smoothing s = smoothChecker->CanSmooth( tri,neighbor1, minCreaseAngle );

		if ( s != Smooth_None )
		{
			neighborGroups[neighbor1->group].push_back(tri->myID);
            flipGroups[ neighbor1->group ].push_back( s );
	...
	{
        assert( neighbor2->group < neighborGroups.size() );

        Smoothing s = smoothChecker->CanSmooth(tri,neighbor2, minCreaseAngle );

		if( s != Smooth_None )
		{
			neighborGroups[neighbor2->group].push_back(tri->myID);
            flipGroups[ neighbor2->group ].push_back( s );
	...
	if(tri->group == NO_GROUP)
	{
        ...
        TriangleList tlist;
        assert( tri->myID != 0xffffffff );
        tlist.push_back( tri->myID );
		neighborGroups.push_back( tlist );
        std::vector< int > f;
        f.push_back( Smooth_Normal );
        flipGroups.push_back( f );
	}
	assert((tri->group != NO_GROUP) && "error!: tri should have a group set");
	tri->handled = true;

	//continue growing our group with each neighbor.
	BuildGroups(neighbor1,possibleNeighbors,neighborGroups,flipGroups,theVerts,smoothChecker,minCreaseAngle);
	BuildGroups(neighbor2,possibleNeighbors,neighborGroups,flipGroups,theVerts,smoothChecker,minCreaseAngle);
}
...
//	unsigned int i;
	...
	for( size_t i = 0 ; i < theVerts.size() ; ++i)
		mappingNewToOldVert.push_back(i);

	m_originalNumVerts = theVerts.size();

	//set up our triangles
	for( size_t i = 0; i < theIndices.size() ; i += 3)
	{
		Triangle t;
...
/*		//if( WeightNormalsByArea != 1.0f )
		{
			...
			Vec3 finalNorm = (normalizedNorm * (1.0f - WeightNormalsByArea))
							+ (t.normal * WeightNormalsByArea);
            //Vec3Normalize( &t.normal, &finalNorm );
            //t.normal = finalNorm;

		}
        //else
        {
            //Vec3Normalize( &t.normal, &t.normal );
        }
*/
        t.normal.Normalize();
	}
						...
						std::vector< Vertex >&   theVerts )
{
	...
	for(size_t i = 0 ; i < len ; ++ i )
	{
		...
		assert( (theVerts[i].normal.Length() > 0.00001f) && 
		...
		Vec3 newT = tmpTan - (tmpNorm * tmpNorm.Dot(tmpTan));
		Vec3 newB = tmpBin - (tmpNorm * tmpNorm.Dot(tmpBin))
							- (newT * newT.Dot(tmpBin));

		newT = theVerts[i].tangent;
		newT.Normalize();
		newB = theVerts[i].binormal;
		newB.Normalize();
		...
		{	
			...
			else
			{
				...
				if (xAxis.Dot(theVerts[i].normal) <  yAxis.Dot(theVerts[i].normal))
				{
...
//	size_t index;
	for( size_t index = 0 ;  index < theIndices.size() ; index += 3)
	{
		//for each triangle
...
MeshMender::Triangle::Triangle()
{
	indices[0] = 0xffffffff;
	indices[1] = 0xffffffff;
	indices[2] = 0xffffffff;

	//per face values
    normal = Vec3( 0.0f, 0.0f, 0.0f );
    tangent = Vec3( 0.0f, 0.0f, 0.0f );
    binormal = Vec3( 0.0f, 0.0f, 0.0f );
