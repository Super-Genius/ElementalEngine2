...
{
	LPDIRECT3DPIXELSHADER9 pShader = m_PixelShader;
	if (pDefines)
	{
	...
	cdx9Rend->SetPixelShader( pShader );
	return true;
}
...
bool CPixelShader::SetConstant( DWORD hash, float * data, int num )
{
	...
	CDX9Renderer * cdx9Rend = (CDX9Renderer *)m_Renderer;
	return cdx9Rend->SetPixelShaderConstantF( shaderconstant, data, num );
}
...
bool CPixelShader::SetConstant( DWORD hash, float x, float y, float z, float w )
{
	...
	data[2] = z;
	data[3] = w;
	return cdx9Rend->SetPixelShaderConstantF( shaderconstant, data, 1 );
}
...
bool CPixelShader::LoadVariant( IShaderDefines *pDefines, bool bCompile )
{
	...
	if (pDefines == NULL && m_PixelShader != NULL)
		return true;
